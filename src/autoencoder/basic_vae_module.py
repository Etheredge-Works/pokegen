...
class VAE(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lr = lr
        self.kl_coeff = min(0.01, kl_coeff)
        if kl_warmup == 0:
            self.kl_coeff = kl_coeff
            kl_warmup = 1
        self.max_kl_coeff = kl_coeff
        self.kl_rate = kl_coeff / kl_warmup
        ...
        if enc_type == "custom":
            self.encoder = ConvEncoder(input_shape=(64, 64, 3), latent_shape=self.enc_out_dim, final_activation=torch.tanh)
            self.decoder = ConvDecoder(latent_shape=self.enc_out_dim, final_activation=torch.tanh)
            print("Using custom encoder and decoder")
        elif enc_type in valid_encoders:
    ...
    def training_epoch_end(self, outputs) -> None:
        self.kl_coeff = min(self.max_kl_coeff, self.kl_coeff + self.kl_rate)
        self.log_dict({"kl_coeff": self.kl_coeff}, on_epoch=True)
